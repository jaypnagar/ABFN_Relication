...
filename =os.path.join(wrd,r"Data\appendix\patent_science_python_topq_fullSample.dta")
print(filename)
...
def E_science_estimate(R, sigma_u):
    ...
    """
    theta1 = np.sqrt(np.exp(0.0236494 )-1)
...
def E_nonscience_estimate(R, sigma_u):
    ...
    :return: float
        Estimated value of E(v|R)
    """
    theta1 = np.sqrt(np.exp(0.0174315 )-1)
    theta2 = np.sqrt(np.exp(0.0236494 )-1)
...
#Save 
filename_save =os.path.join(wrd,r"Data\appendix\Multipatent_fromPython_topq_fullSample.dta")
print(filename_save)
