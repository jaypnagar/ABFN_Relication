...
filename =os.path.join(wrd,r"Data\appendix\patent_pythonteamsize_topq_invt.dta")
print(filename)
...
def E_science_estimate(R, sigma_u):
    ...
    """
    theta1 = np.sqrt(np.exp(.0212011)-1)
    theta2 = np.sqrt(np.exp(.0169341 )-1)
...
def E_nonscience_estimate(R, sigma_u):
    ...
    :return: float
        Estimated value of E(v|R)
    """
    theta1 = np.sqrt(np.exp(.0169341)-1)
    theta2 = np.sqrt(np.exp(.0212011)-1)
...
#Save 
filename_save =os.path.join(wrd,r"Data\appendix\Multipatent_pythonteamsize_topq_invt.dta")
print(filename_save)
