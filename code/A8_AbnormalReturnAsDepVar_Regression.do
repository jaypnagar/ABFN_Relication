...
keep permno_adj bdate date patentaward npatents pat_atonenonus_invt_any pat_allnonusa_invt_any pat_majornonusa_invt_any patent_largeteam_any patent_largeteam_topq_any patent_multiteam_any patents_cscience_any patents_topq_cscience_any patents_top4q_cscience_any
...
rangejoin patent_award_date event_start event_end using "Data\ret.dta", by(permno_adj) keepusing(patentaward previous_award_date next_award_date npatents pat_atonenonus_invt_any pat_allnonusa_invt_any pat_majornonusa_invt_any patent_largeteam_any patent_largeteam_topq_any patent_multiteam_any patents_cscience_any patents_topq_cscience_any patents_top4q_cscience_any)
sort permno_adj bdate
duplicates drop permno_adj bdate, force

foreach var of varlist patent_award_date patentaward npatents pat_atonenonus_invt_any pat_allnonusa_invt_any pat_majornonusa_invt_any patent_largeteam_any patent_largeteam_topq_any patent_multiteam_any patents_cscience_any patents_topq_cscience_any patents_top4q_cscience_any {
...
bys permno : egen fy_patents_topq_cscience_any = max(patents_topq_cscience_any)
bys permno : egen fy_no_patents_topq_cscience_any = min(patents_topq_cscience_any) 

bys permno : egen fy_pat_allnonusa_invt_any = max(pat_allnonusa_invt_any)
bys permno : egen fy_no_pat_allnonusa_invt_any = min(pat_allnonusa_invt_any) 

bys permno : egen fy_patent_largeteam_topq_any = max(patent_largeteam_topq_any)
bys permno : egen fy_no_patent_largeteam_topq_any = min(patent_largeteam_topq_any) 

bys permno : egen fy_pa_topvi_high = max(pa_topvi_high)
bys permno : egen fy_no_pa_topvi_high = min(pa_topvi_high) 



*************Baseline Regression*******************

sum patents_topq_cscience_any patent_largeteam_topq_any pat_allnonusa_invt_any pa_topvi_high
...
label var pa_topvi_high "Patent day: High VI Firm "
...
est clear
eststo sumstats: qui estpost sum abnret event patents_topq_cscience_any patent_largeteam_topq_any pat_allnonusa_invt_any pa_topvi_hig, d
...
eststo:reghdfe abnret event patent_largeteam_topq_any lmktvaluel npatents if fy_event == 1 & fy_noevent == 0 & fy_patent_largeteam_topq_any == 1 & fy_no_patent_largeteam_topq_any == 0, absorb(permno_adj bdate) cluster(permno)
qui sum `e(depvar)' if e(sample)
estadd scalar mean_dv = r(mean)  
estadd local Firms = "Yes"
estadd local date = "Yes"

eststo:reghdfe abnret event pat_allnonusa_invt_any lmktvaluel npatents if fy_event == 1 & fy_noevent == 0 & fy_pat_allnonusa_invt_any == 1 & fy_no_pat_allnonusa_invt_any == 0, absorb(permno_adj bdate) cluster(permno)
qui sum `e(depvar)' if e(sample)
estadd scalar mean_dv = r(mean)  
estadd local Firms = "Yes"
estadd local date = "Yes"

eststo:reghdfe abnret event patents_topq_cscience_any lmktvaluel npatents if fy_event == 1 & fy_noevent == 0 & fy_patents_topq_cscience_any == 1 & fy_no_patents_topq_cscience_any == 0, absorb(permno_adj bdate) cluster(permno)
qui sum `e(depvar)' if e(sample)
estadd scalar mean_dv = r(mean)  
estadd local Firms = "Yes"
estadd local date = "Yes"

eststo:reghdfe abnret event pa_topvi_high lmktvaluel npatents if fy_event == 1 & fy_noevent == 0 & fy_pa_topvi_high == 1 & fy_no_pa_topvi_high == 0 &  merge_vertinteg==3, absorb(permno_adj bdate) cluster(permno)
qui sum `e(depvar)' if e(sample)
estadd scalar mean_dv = r(mean)  
estadd local Firms = "Yes"
estadd local date = "Yes"
...
esttab using "Output\Table_PartB_v1_baselineReturn_sample.tex", replace cells(b(star fmt(%12.4f)) se(par)) ///
	...
	mgroups("Abnormal Return" , pattern(1 0 0  0  ) ///
      ...
      span erepeat(\cmidrule(lr){@span})) ///
       order(event patent_largeteam_topq_any pat_allnonusa_invt_any patents_topq_cscience_any  pa_topvi_high mktvaluel npatents)
